...
int	main(void)
{
	...
	//g.map.map =	strdup("1111111111111111100000011000000110000E01100000011111111111111111");
	g.map.map =	strdup("1111111111111111110000111101001111000E11110100111111111111111111");
	...
	g.mlx = mlx_init();
	//g.h = WY;//g.map.mapH * TL;// game/window height = 8 tiles * 64 px = 512 px;
	//g.w = WX;//g.map.mapW * TL;// game/window width = 8 tiles * 64 px = 512 px;
	g.win = mlx_new_window(g.mlx, WX, WY, "cub3D");//g.win = mlx_new_window(g.mlx, g.w, g.h, "cub3D");
	...
	g.img3.h = WY;//g.map.mapH * PX3;//768;//1536;
	...
	//mlx_loop_hook(g.mlx, &render_frames, &g);
	mlx_loop(g.mlx);
}
